...
Run this exact Bash command, substituting the extracted values:
  ...
  node "${CLAUDE_PLUGIN_ROOT}/hooks/add-sdk.js" <sdk-name> <docs-url>

If the user also provided a third argument (an import name that differs from the sdk-name, e.g. for scoped packages like @scope/pkg), pass it as a third argument.

Report the output of the script verbatim. If the script errors, show the error and stop — do not attempt to edit watchlist.json manually.

After a successful add, tell the user:
